#####
##### GLEN-forced Southern Mooring — profile plots
#####
# Rows = winter year; columns = turbulence closure.
# Each panel shows snapshots at days 0, 7, 15, 22, 30 (month 1) or
# 30, 37, 45, 52, 60 (month 2).
# Row labels include mean Q_T (W/m²) and Q_U (m²/s²) averaged over each 30-day window.
# Q_T uses the gap-filled GLEN NetCDF (Q_precomp + LW_up from T_sfc).
#
# Inputs  : data/TURB_outputs/southern_mooring_GLEN_forced/winter<YEAR>/
#               <closure>[_coare_wind]_winter<YEAR>.jld2
#           figure_data/lake_superior_southern_mooring/
#               lake_superior_southern_mooring_winter_start_dates.csv  (isothermal date)
#           /lcrc/project/HSOFS_Ensemble/COMPASS_GLM/GLEN/
#               US_StannardRockSuperior_processed_halfhourly_qc_gapfilled.nc  (forcing)
# Outputs : figures/TURB_southern_mooring_GLEN_forced_{T,u,N2}_profiles_{month1,month2}_{direct,coare_wind}.pdf
#
# Usage:
#   julia plot_TURB_southern_mooring_GLEN_forced_profiles.jl               # direct (default)
#   julia plot_TURB_southern_mooring_GLEN_forced_profiles.jl coare_wind
#####
...
TURB_DIR   = joinpath(@__DIR__, "..", "data", "TURB_outputs", "southern_mooring_GLEN_forced")
...
obs_sm = if isfile(OBS_SM_FILE)
    ...
    @warn "Southern mooring obs not found — run process_lake_superior_southern_eastern_moorings.jl first"
    nothing
end

winter_years = [2010, 2011]
...
function plot_profiles_GLEN(field_name, xlabel_str, filename;
    ...
    with_theme(theme_latexfonts()) do
        ...
        has_obs = field_name == "Tbar" && !isnothing(obs_sm)
        if has_obs
            obs_T     = forcing_key == :month1 ? obs_sm.T1     : obs_sm.T2
            obs_T_raw = forcing_key == :month1 ? obs_sm.T1_raw : obs_sm.T2_raw
            z_obs_sm  = -obs_sm.dep
            for (row, year) in enumerate(winter_years)
                wy_idx = findfirst(==(year), obs_sm.wys)
                isnothing(wy_idx) && continue
                T_interp = obs_T[:, wy_idx]
                T_raw    = obs_T_raw[wy_idx]
                z_raw    = -obs_sm.dep_raw[wy_idx]
        ...
        Label(fig[0, 1:Nclosures],
              "Southern Mooring — GLEN-forced ($(month_days)-day mean forcing shown per row)";
...
for (snap_days, fkey, suffix) in [
    ...
    plot_profiles_GLEN("Tbar",
                       L"T \; (^\circ\mathrm{C})",
                       "TURB_southern_mooring_GLEN_forced_T_profiles_$(suffix)_$(FORCING_SOURCE).pdf";
    ...
    plot_profiles_GLEN("ubar",
                       ...
                       "TURB_southern_mooring_GLEN_forced_speed_profiles_$(suffix)_$(FORCING_SOURCE).pdf";
                       snapshot_days = snap_days, forcing_key = fkey,
                       xlims_range = (0.0, 0.2),
                       z_nodes = zC)
    ...
    plot_profiles_GLEN("N²bar",
                       L"N^2 \; (\mathrm{s^{-2}})",
                       "TURB_southern_mooring_GLEN_forced_N2_profiles_$(suffix)_$(FORCING_SOURCE).pdf";
                       snapshot_days = snap_days, forcing_key = fkey,
                       z_nodes = zF)
end
