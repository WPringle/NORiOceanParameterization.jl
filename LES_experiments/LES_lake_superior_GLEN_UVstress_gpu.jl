...
Usage:
    ...
    Options:
        ...
        --Cd            Quadratic bottom drag coefficient (dimensionless), applied as a
                        `bottom` FluxBoundaryCondition on u/v: τ = -Cd*|u|*u (and the v
                        analogue), using the near-bed (k=1) velocity.  Default 0.0 = no
                        drag (free-slip lake bed, as before).  Typical natural lakebed
                        value ~2-3e-3.
...
# velocity.  u and v live at different horizontal points (Face,Center) / (Center,Face),
# so the cross-component is interpolated onto the BC's own point before computing speed.
# Sign convention verified directly: as a `bottom` FluxBoundaryCondition, this form
# decelerates the near-bed flow (not accelerates it) — no extra sign flip needed.
...
FILE_NAME = "LES_GLEN_winter$(winter_year)$(SRC_TAG)$(CLOSURE_TAG)$(DRAG_TAG)_UVstress_Lxy$(round(Int,Lx))_Lz$(round(Int,Lz))_Nxy$(Nx)_Nz$(Nz)"
