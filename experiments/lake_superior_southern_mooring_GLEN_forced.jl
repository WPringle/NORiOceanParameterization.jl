...
#
# This uses the gap-filled product, which provides momentum flux and turbulent heat
# fluxes DIRECTLY — no bulk-formula computation is needed here.  The only term still
...
# converted to kinematic stress τ = -momentum_flux / ρ₀ (negative = into lake surface).
#
...
#
# Usage:
#   julia lake_superior_southern_mooring_GLEN_forced.jl <winter_year> [closure_name] [forcing_source]
#   e.g.  julia lake_superior_southern_mooring_GLEN_forced.jl 2010
#         julia lake_superior_southern_mooring_GLEN_forced.jl 2010 CATKE
#         julia lake_superior_southern_mooring_GLEN_forced.jl 2010 CATKE coare_wind
#         julia lake_superior_southern_mooring_GLEN_forced.jl 2010 coare_wind   # all closures
#
...
#               <closure>[_coare_wind]_winter<YEAR>.jld2        (averaged profiles)
#               <closure>[_coare_wind]_winter<YEAR>_LMO.jld2    (Monin-Obukhov length)
#               forcing[_coare_wind]_winter<YEAR>.jld2          (GLEN forcing arrays used)
#           (the `direct` source keeps the original, untagged filenames.)
#####
...
const SRC_TAG = FORCING_SOURCE == "direct" ? "" : "_coare_wind"    # filename tag
...
function run_simulation(model, closure_name, OUTPUT_PATH)
    ...
    mkpath(OUTPUT_PATH)
    filename = joinpath(OUTPUT_PATH, "$(closure_name)$(SRC_TAG)_winter$(winter_year).jld2")
    ...
    lmo_file = joinpath(OUTPUT_PATH, "$(closure_name)$(SRC_TAG)_winter$(winter_year)_LMO.jld2")
