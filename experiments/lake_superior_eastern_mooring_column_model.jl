...
#####
##### Lake Superior Eastern Mooring — idealized column model
#####
# Location: 47° 32.2' N
# Depth:    H = 212 m
# Season:   cooling cycle with wind forcing
# Purpose:  compare k-ε and CATKE closures under three
#           typical autumn wind speeds (6, 10, 14 m/s) and three
#           mean surface cooling rates (150, 250, 350 W/m²)
#####

model_architecture = CPU()

# ── Grid ────────────────────────────────────────────────────────────────────
const Nz = 53      # (for dz = 4 m) 
const Lz = 212.0   # m
...
const lat = 47.0 + 19.3 / 60.0          # decimal degrees (I used the western mooring for LEs sims)
...
const QT_mean_cases = [150.0, 250.0, 350.0]   # W/m²

# Three wind speed cases: typical autumn conditions on Lake Superior
const wind_cases = [
    ("U6ms",  6.0),    # weak wind
    ("U10ms",  10.0),    # intermediate wind
    ("U14ms",  14.0),    # strong wind
]
...
function run_simulation(model, Qᵁ, Q_mean, Q_amp, wind_label, closure_name, OUTPUT_PATH)
    ...
    @info "Running $closure_name | wind = $wind_label | Qᵀ = diurnal (150–350 W/m²) | Qᵁ = $(round(Qᵁ, sigdigits=4))"
...
OUTPUT_PATH = joinpath(@__DIR__, "..", "data", "TURB_outputs","eastern_mooring")
